...
function getCopy(locale: string) {
  ...
  if (locale === "ja") {
    return {
      ...
      introBody:
        "寝室は一日の始まりと終わりを過ごす場所なので、ここにある clutter はほかの部屋以上に重く感じやすいものです。床に置いた服、物が多いナイトテーブル、あふれたベッド下収納、もう着ない服が混じったクローゼットは、眠る前も起きた直後も小さなストレスになります。いちばん効くのは収納を足すことではなく、負担を増やしているものを取り除くことです。",
      ...
      steps: [
        ...
        ["4. ベッド下収納を後回しゾーンにしない", "ベッド下は『あとで決める』物の避難場所になりがちです。見えない clutter でも注意力と空間を奪うので、意図を持って見直します。"],
        ["5. 部屋を忙しく見せる visual stressor を減らす", "多すぎるクッション、ほこりをかぶった decor、見えたままのコード、服置き場になった椅子は、寝室を休む場所ではなく作業待ちの場所に変えてしまいます。"],
        ["6. 過去の自分に結びつくものを分ける", "昔の制服、今の暮らしに合わない服、高かった失敗買い、感情の重い keepsake は、日常品と混ぜたままにせず意識して見直します。"],
        ["7. 行動には checklist、迷う物には decision guide を使う", "今すぐ判断できる物は片付け、迷う物は期限つきの decision guide に移します。maybe pile を寝室に残し続けないことが大切です。"],
      ],
      ...
      toolTitle: "Bedroom Checklist から始める",
      toolDesc:
        "このインタラクティブなチェックリストは、専用の Bedroom Checklist ページと同じライブデータを使っています。どちらのページからでも項目のチェック、追加、進捗の保存ができます。",
      toolPage: "Bedroom Checklist ページを開く",
      ...
      faqTitle: "寝室の片付け FAQ" as string | undefined,
      faqs: [
        ["寝室を最速で片付けるには？", "まず床を空け、ナイトテーブルを整え、クローゼットから明らかな no を抜きます。この 3 つだけでも 15 分で部屋の印象がかなり変わります。"],
        ["圧倒されて寝室を片付けられないときは？", "部屋全体ではなく surface を 1 か所だけ選び、10 分だけタイマーをかけてください。全部終わらなくても、見える変化があるほうが次につながります。"],
