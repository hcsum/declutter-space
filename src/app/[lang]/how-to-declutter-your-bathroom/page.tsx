...
function getCopy(locale: string): GuideCopy {
  ...
  if (locale === "ja") {
    return {
      ...
      heroSubtitle:
        "浴室の clutter は、小さくて似た物が多く、残す理由もつけやすいので一気に増えます。大切なのは毎日使う物を見やすくし、あふれたストックを抑えて、朝の時点で窮屈に感じない浴室に戻すことです。",
      introTitle: "浴室の clutter は backup clutter と小物の積み重なりになりやすい",
      introBody:
        "サンプル、重複した toiletries、期限が近い製品、古いタオル、空ボトル、必要以上のストックは、一つひとつは小さくても浴室ではすぐに圧迫感になります。いちばん効くリセットは、いらない物の量を先に減らすことです。もう不要な物のために organizer を増やしても、根本は軽くなりません。",
      stepsTitle: "浴室を片付ける 7 つのステップ",
      steps: [
        ["1. 期限切れ・劣化・明らかに使わない物を捨てる", "期限切れの薬、日焼け止め、skincare、メイク、サンプル、においや質感や色が変わった物から始めましょう。"],
        ["2. 同じカテゴリの物をまとめる", "haircare、skincare、oral care、掃除用品、救急用品をそれぞれ集めると、重複や買いすぎが一気に見えます。"],
        ["3. 使っている物だけをいちばん取りやすい場所に置く", "カウンターやシャワー棚には今使っている物だけを置きます。ストックはあってもよいですが、日用品と競わせないことが大切です。"],
        ["4. サンプルと travel-size を減らす", "小さい物は harmless に見えても、長期的な clutter になりがちです。近いうちに使う物か旅行に持っていく物だけ残します。"],
        ["5. 使い古したタオル、シャワーカーテン、バスマットを見直す", "こうした布物は清潔感に大きく影響します。黄ばみ、におい、ほつれ、使い心地の悪さがあるなら、役目を終えている可能性が高いです。"],
        ["6. 引き出しや棚の random small items を整える", "ヘアゴム、綿棒、カミソリ、予備電池、説明書、おまけ類は混ざるとすぐ探しにくくなります。"],
        ["7. ストックには明確な上限を設ける", "予備の歯磨き粉、トイレットペーパー、シャンプー、掃除用品は便利ですが、見える境界がないと収納が静かに広がり続けます。"],
      ],
      removeTitle: "浴室で最初に減らしたいもの",
      removeItems: [
        "期限切れの薬や personal care 製品",
        "肌や髪に合わないまま残っている toiletries",
        "使っていないサンプルや travel-size",
        "空ボトル、空箱、包装材",
        "黄ばんだ・におう・傷んだタオルや布物",
        "あふれた予備の toiletries や cleaners",
      ],
      ...
      mistakes: [
        "ストックと今使う物を同じ場所に混ぜる",
        "行くか分からない旅行のためにサンプルや travel-size を残し続ける",
        "小さいからという理由でカウンターに物を出しすぎる",
        "期限切れの skincare や薬を harmless clutter のように扱う",
      ],
      keepTitle: "浴室で見える場所に置くべきもの",
      keepBody:
        "朝と夜の routine で実際に使っている製品だけを、手の届きやすい場所に残しましょう。ストック、来客用、たまにしか使わない物は分けておくと、浴室が『持ち物全体』ではなく『今日の支度』を映すようになります。",
      ...
      faqTitle: "浴室の片付け FAQ",
      faqs: [
        ["予備の toiletries はどれくらい持てばいい？", "決めた 1 つのストックゾーンに収まる量だけで十分です。そこからあふれたら、便利さではなく clutter になっています。"],
        ["浴室のカウンターは完全に空にすべき？", "いいえ。毎日実際に使う物は置いて問題ありません。目指すのはホテルの見た目ではなく、使いやすい明快さです。"],
        ["浴室では何から片付けるのが早い？", "期限切れの製品、使っていないサンプル、重複している物からです。routine を崩さずに一番早く空間を作れます。"],
      ],
      toolTitle: "Bathroom Checklist から始める",
      toolDesc:
        "このインタラクティブなチェックリストは、専用の Bathroom Checklist ページと同じライブデータを使っています。どちらのページからでも項目のチェック、追加、進捗の保存ができます。",
      toolPage: "Bathroom Checklist ページを開く",
