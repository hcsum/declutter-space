  ...
  if (locale === "zh") {
    return {
      ...
      steps: [
        ...
        ["7. 用清单执行，再用小复盘维持结果", "整理衣柜不是一次性项目。先用清单完成第一轮，再定期快速复查，避免反复堆回去。"],
      ],
  ...
  if (locale === "ja") {
    return {
      ...
      steps: [
        ...
        ["3. ランダムではなくカテゴリごとに見る", "トップス、ボトムス、ジャケット、靴、バッグ、小物を別々に見ると、重複やパターンがずっと見えやすくなります。"],
        ["4. 今の生活に合わない服を減らす", "悪い服ではなくても、昔の仕事、昔の習慣、昔のスタイルに結びついた服は、今の自分には不要かもしれません。"],
        ["5. 罪悪感で買った服と正直に向き合う", "お金はすでに払っています。高かった失敗を残しても元は取れず、クローゼットの家賃だけ払い続けることになります。"],
        ["6. 実際によく着る物に一等地を渡す", "取り出しやすい場所は、現実の 1 週間を支える服、靴、バッグのために使いましょう。"],
        ["7. チェックリストと短い見直しで維持する", "クローゼットの散らかりは少しずつ戻ります。最初のリセットを終わらせるにはチェックリスト、戻りすぎを防ぐには短い定期レビューが役立ちます。"],
      ],
      ...
      emotionTitle: "よいクローゼットは判断疲れを下げる",
      emotionBody:
        "役立つクローゼットは、たくさん入るから優秀なのではありません。不確実さが少ないから優秀なのです。今の体、今の習慣、今のスタイルを反映しているクローゼットなら、着替えは小さな議論ではなく、もっと静かな作業になります。",
      mistakesTitle: "クローゼットの片付けでよくある失敗",
      mistakes: [
        "明らかに不要な物を抜く前に、すべて試着しようとする",
        "実際には選ばないのに実用的に感じるという理由で定番服を重複保管する",
        "現実の 1 週間ではなく、理想の自分用の服を中心に整える",
        "毎日のお気に入りではなく、罪悪感で買った服に一等地を渡す",
      ],
      keepTitle: "いちばん取りやすいクローゼットの場所に置くべきもの",
      keepBody:
        "最もアクセスしやすい場所には、普段の 1 週間で着る服、履く靴、使うバッグを置きましょう。よそ行きの服、思い出の品、季節外のあふれた分は残してもかまいませんが、毎日の支度を支える部分を圧迫しないようにします。",
      quickResetTitle: "15 分でクローゼットを片付ける方法",
      quickResetSteps: [
        "傷み、シミ、明らかな着心地の悪さがある服を抜く",
        "トップス、ボトムス、靴、バッグのどれか 1 カテゴリを素早く見る",
        "着替えのたびに飛ばしている服を分ける",
        "今の生活と今の体に合う服だけを戻す",
      ],
      faqTitle: "クローゼットの片付けのよくある質問",
      faqs: [
        ...
        ["全部まだ着られるのに、なぜか窮屈に感じるのは？", "サイズは 1 つの基準にすぎません。実際に選ばない、好きではない、今の暮らしで使わないなら、着られても不要な物であることはあります。"],
      ],
      toolTitle: "クローゼットチェックリストから始める",
      toolDesc:
        "このインタラクティブなチェックリストは、専用のクローゼットチェックリストページと同じライブデータを使っています。どちらのページからでも項目のチェック、追加、進捗の保存ができます。",
      toolPage: "クローゼットチェックリストのページを開く",
